...
async def func(client: httpx.AsyncClient, url: str):
    try:
...
async def _(bot: Bot, event: MessageEvent):
    ...
    if isinstance(event, GroupMessageEvent):
        ...
        if api == "Lolicon API":
            ...
            if not R18 or not Tag or (tag := is_MirlKoi_tag(Tag)):
                Tag = tag
                api = "MirlKoi API"
                setufunc = MirlKoi
            else:
    ...
    if N <= 3:
        ...
        for i in range(N):
            msg_list.append(
                {"type": "node", "data": {"name": Bot_NICKNAME, "uin": event.self_id, "content": MessageSegment.image(file=image_list[i])}}
            )
        if isinstance(event, GroupMessageEvent):
